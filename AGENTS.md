...
macOS 菜单栏应用 **cc-monitor-bar**，用于本地监控 Codex 的使用情况：活跃会话追踪、Token 用量估算、工具调用统计、每日聚合分析。
...
**服务层** (`Services/`) — 核心业务逻辑：
- `ClaudeDataReader` — 读取 `~/.Codex/` 下的数据文件（stats-cache.json、history.jsonl、sessions/*.json、projects/*/*.jsonl）
...
| 路径 | 用途 |
|------|------|
| `~/.Codex/stats-cache.json` | Token 统计基线 |
| `~/.Codex/history.jsonl` | 会话历史索引 |
| `~/.Codex/sessions/*.json` | 活跃会话元数据 |
| `~/.Codex/projects/*/*.jsonl` | 项目会话转录 |

## 设计文档

详细规格和实施计划在 `docs/superpowers/` 下，数据源调研在 `docs/Codex-data-survey.md`。
